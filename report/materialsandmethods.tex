...
\subsection{Autoencoder Network Architecture}
...
\mychapterref{sec:ExperimentsandResults} introducing an $\epsilon$-ball also improves
...
While the general approach derived in \mysecref{sec:Approach_GPF} can be trivially
...
\ac{SISR} problem is shown in \myfigref{fig:architecture}. The high-dimensional
...
\myfigref{fig:comparison_scaling_iter_vs_direct} the model can be trained more
...
\subsubsection*{Video-Super-Resolution}
As pointed out in \mychapterref{sec:RelatedWork} the challenge of \ac{VSR}
...
As shown in \mychapterref{sec:RelatedWork} most approaches tackling the
...
The most promising approach is displayed in \myfigref{fig:architecture_video}
...
\subsubsection*{Image Colorization}
